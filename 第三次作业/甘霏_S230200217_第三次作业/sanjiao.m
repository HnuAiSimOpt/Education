%% 有限元作业：使用三角形单元求解平面问题。
%学号：S230200217  姓名：甘霏
% 程序计算思路：划分网格、求单元刚度矩阵、集成总体刚度矩阵、求解位移、回代求解应力。
clear;
clc;
%% 输入求解条件
a=8;
...
F=-2; %施加的力
h=0.25; %单元边长
[Node_info,Ele_info,Num_nodes]=Meshing(a,b,h); %划分网格
...
figure(1);Plot_u(u,Node_info,Num_nodes); %绘制位移分布云图
...
%% 网格划分（输入物体边长和单元边长，输出包含节点编号、节点坐标的节点信息与包含单元编号、单元包含节点编号的单元信息）
function [Node_info,Ele_info,Num_nodes]=Meshing(a,b,h)
axis equal
hold on
%节点信息
Num_nodes=0;
...
row1_x=find(Node_info(:,2)==8);
row1_y=find(Node_info(:,3)==4);
...
%% 后处理函数 绘制节点位移云图
function []=Plot_u(u,Node_info,Num_nodes)
u1=[];
u2=[];
for i=1:Num_nodes
 u1=[u1;u(2*i-1)];
