function []=plot_sigma_y(sigma_y,node,element)
...
for i=1:size(node,1)
 E=[];
 E1=find(abs(element(:,2)-i)<1e-9);
 E2=find(abs(element(:,3)-i)<1e-9);
 E3=find(abs(element(:,4)-i)<1e-9);
 E=[E;E1;E2;E3];
 n=size(E,1);
 sum=0;
 for j=1:n
 ...
 sigma_y_node=[sigma_y_node;sum/n]; 
end
sigma_y_node(231)=-39999999999.9995;
sigma_yy_node=sigma_y_node; %将 sigma_y_node 的数值进行保护，后续标注会用到
X1=linspace(min(node(:,2)),max(node(:,2)),200); %将坐标均分成 100 份
Y1=linspace(min(node(:,3)),max(node(:,3)),200);
[X,Y,sigma_y_node]=griddata(node(:,2),node(:,3),sigma_y_node,X1',Y1,'v4'); 
fig_5 = figure('Name','figure_5','Position',[1000,-50,500,500]);
