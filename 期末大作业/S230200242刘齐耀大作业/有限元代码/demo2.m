...
%                        一阶六面体单元主程序
%                         S230200242 刘齐耀 
%  demo1: 立放，125N双集中力载荷, 网格模型信息: job_2.inp           
%***********************************************************************
function demo2()
%读取inp文件获得节点坐标信息Nodes及单元信息Elements
[Nodes, Elements] = Readmesh( 'job_2.inp' );
% 外力矩阵 Forces=[受力节点  受力方向(1,2,3分别代表x,y,z)  外力大小]  外力节点的编号在inp文件里面找
Forces=[2 3 125; 3 3 125]; 
%约束节点的编号在inp文件里面找
...
% 输出结果
OutputTXT = fopen('demo2_Results.txt','w'); %打开一个可写文件，用于写入计算结果
OutputResults(OutputTXT,Nodes,Elements,D,U)%调用输出结果文件
fclose(OutputTXT);
edit('demo2_Results.txt')
end