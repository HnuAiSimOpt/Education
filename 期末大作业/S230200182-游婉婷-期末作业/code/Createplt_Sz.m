function []=Createplt_Sz(node,element_solid,sz)
eles_num=size(element_solid,1);
nodes_num=size(node,1);
for i=1:1860
    Sz(element_solid(i,2))=sz(i);
    Sz(element_solid(i,3))=sz(i);
    Sz(element_solid(i,4))=sz(i);
    Sz(element_solid(i,5))=sz(i);
    Sz(element_solid(i,6))=sz(i);
    Sz(element_solid(i,7))=sz(i);
    Sz(element_solid(i,8))=sz(i);
    Sz(element_solid(i,9))=sz(i);
end
fid=fopen('result_Sz.plt','w');
fprintf(fid,'TITLE="test case governed by poisson equation"\n');
fprintf(fid,'VARIABLES="x""y""z""sigaz"\n');
fprintf(fid,'ZONE N=%8d,E=%8d,ET=BRICK,F=FEPOINT\n',nodes_num,eles_num);
for i = 1:nodes_num
    fprintf(fid,'%16.6e%16.6e%16.6e%16.6e\n',node(i,2),node(i,3),node(i,4),Sz(1,i));
end
for i=1:eles_num
